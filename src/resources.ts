// MCP Resource Handlers for the Rootly MCP server
// Exposes the MCP Apps (SEP-1865) incident-card UI via ListResources and
// ReadResource handlers. The card HTML is embedded at build time
// (src/generated/incident-card-html.ts) so it serves identically from stdio,
  ...
  return [
    {
      ...
      name: 'Rootly Incident Card',
      description: 'Interactive MCP Apps card rendering a Rootly incident',
...
export function readResource(uri: string): McpResourceContent {
  if (uri === INCIDENT_CARD_RESOURCE_URI) {
    return {
      ...
      text: applyBrandInjection(INCIDENT_CARD_HTML, resolveBrandFromEnv()),
    };
  }
